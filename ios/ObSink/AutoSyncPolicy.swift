...
/// older than `staleAfter`; never while busy, keyless, deleted on the
/// server or holding conflicts (those need the user).
enum AutoSyncPolicy {
    ...
    static func shouldSync(_ state: VaultState, now: Date) -> Bool {
        guard state.phase == .idle, state.hasStoredKey, !state.deletedOnServer, state.conflicts == 0 else {
            return false
        }
